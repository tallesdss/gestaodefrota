...
List<ExpenseCategory> _buildInitialCategories() => [
  ExpenseCategory(
    ...
    subgroups: [
      ExpenseSubgroup(name: 'Motor', items: [
      ...
      ExpenseSubgroup(name: 'Suspensão', items: [
      ...
      ExpenseSubgroup(name: 'Freios', items: [
      ...
      ExpenseSubgroup(name: 'Transmissão', items: [
        ExpenseSubcategory(name: 'Troca de óleo do câmbio'),
        ExpenseSubcategory(name: 'Troca de embreagem (disco, plato e rolamento)'),
      ...
      ExpenseSubgroup(name: 'Arrefecimento', items: [
      ...
      ExpenseSubgroup(name: 'Elétrica', items: [
        ...
        ExpenseSubcategory(name: 'Troca de lâmpadas (faróis, lanternas, setas)'),
        ExpenseSubcategory(name: 'Troca de sensores (rotação, temperatura, ABS)'),
        ExpenseSubcategory(name: 'Reparo de chicote elétrico'),
      ]),
      ExpenseSubgroup(name: 'Pneus', items: [
      ...
      ExpenseSubgroup(name: 'Ar-condicionado', items: [
  ...
  ExpenseCategory(
    ...
    subgroups: [
      ExpenseSubgroup(name: 'Combustível', items: [
      ...
      ExpenseSubgroup(name: 'Lubrificantes e fluidos', items: [
        ...
        ExpenseSubcategory(name: 'Completar fluido de limpador de para-brisa'),
      ]),
    ],
  ),
  ExpenseCategory(
    ...
    color: Colors.blue,
    subgroups: [
      ExpenseSubgroup(name: 'Limpeza', items: [
      ...
      ExpenseSubgroup(name: 'Funilaria', items: [
      ...
      ExpenseSubgroup(name: 'Pintura', items: [
      ...
      ExpenseSubgroup(name: 'Vidros', items: [
      ...
      ExpenseSubgroup(name: 'Acessórios', items: [
  ...
  ExpenseCategory(
    ...
    subgroups: [
      ExpenseSubgroup(name: 'Impostos e taxas', items: [
      ...
      ExpenseSubgroup(name: 'Seguro', items: [
      ...
      ExpenseSubgroup(name: 'Infrações', items: [
      ...
      ExpenseSubgroup(name: 'Sinistros', items: [
      ...
      ExpenseSubgroup(name: 'Vistoria e transferência', items: [
...
class _ExpenseCategoriesScreenState extends State<ExpenseCategoriesScreen> {
  ...
  int get _totalSubgroups => _categories.fold(0, (s, c) => s + c.subgroups.length);
    ...
    return cat.subgroups
        .map((sg) {
          final filteredItems = sg.items
              ...
              .toList();
          if (filteredItems.isNotEmpty || sg.name.toLowerCase().contains(query)) {
      ...
      builder: (ctx) => AlertDialog(
        ...
        content: TextField(
          ...
          decoration: InputDecoration(
            hintText: hint,
            hintStyle: GoogleFonts.inter(fontSize: 14, color: AppColors.onSurfaceVariant.withAlpha(128)),
            filled: true,
            ...
            focusedBorder: OutlineInputBorder(
              borderRadius: BorderRadius.circular(12),
              borderSide: BorderSide(color: AppColors.primary.withAlpha(102), width: 2),
            ),
          ),
        ),
        actions: [
          TextButton(
            ...
            child: Text('Cancelar', style: GoogleFonts.inter(fontWeight: FontWeight.w600, color: AppColors.onSurfaceVariant)),
          ),
          FilledButton(
            onPressed: () {
            ...
            child: Text('Salvar', style: GoogleFonts.inter(fontWeight: FontWeight.w700)),
          ),
        ],
      ),
    );
      ...
      builder: (ctx) => AlertDialog(
        ...
        actions: [
          ...
          FilledButton(
            ...
            child: Text('Remover', style: GoogleFonts.inter(fontWeight: FontWeight.w700)),
          ),
        ],
      ),
    ) ?? false;
  ...
  void _addSubgroup(int catIndex) async {
    ...
    if (name != null) {
      setState(() {
        _categories[catIndex].subgroups.add(ExpenseSubgroup(name: name, items: []));
  ...
  void _addItem(int catIndex, int subIndex) async {
    ...
    if (name != null) {
      setState(() {
        _categories[catIndex].subgroups[subIndex].items.add(ExpenseSubcategory(name: name));
  ...
  void _editItem(int catIndex, int subIndex, int itemIndex) async {
    final currentName = _categories[catIndex].subgroups[subIndex].items[itemIndex].name;
    final name = await _showTextDialog(
      title: 'Editar Item',
      hint: 'Nome do item',
      initialValue: currentName,
    ...
    if (name != null && name != currentName) {
      setState(() => _categories[catIndex].subgroups[subIndex].items[itemIndex].name = name);
      _showSnack('Item renomeado para "$name"');
    }
  }

  void _deleteItem(int catIndex, int subIndex, int itemIndex) async {
    final itemName = _categories[catIndex].subgroups[subIndex].items[itemIndex].name;
    final confirmed = await _showDeleteDialog(itemName);
    if (confirmed) {
      setState(() => _categories[catIndex].subgroups[subIndex].items.removeAt(itemIndex));
        ...
        children: [
          ...
          Padding(
            ...
            child: Container(
              ...
              child: Row(
                children: [
                  _buildStatChip('${_categories.length}', 'Categorias', Icons.folder_outlined),
                  const SizedBox(width: AppSpacing.xl),
                  _buildStatChip('$_totalSubgroups', 'Subgrupos', Icons.folder_open_outlined),
                  const SizedBox(width: AppSpacing.xl),
                  _buildStatChip('$_totalItems', 'Itens', Icons.list_alt_rounded),
                ],
              ),
            ),
          ),
            ...
            child: filteredCategories.isEmpty
                ? Center(
                    child: Column(
                      ...
                      children: [
                        Icon(Icons.search_off_rounded, size: 48, color: AppColors.onSurfaceVariant.withAlpha(77)),
                        const SizedBox(height: AppSpacing.md),
                        Text(
    ...
    return Row(
      children: [
        ...
        Column(
          ...
          children: [
            Text(value, style: GoogleFonts.manrope(fontSize: 18, fontWeight: FontWeight.w800, color: Colors.white)),
            Text(label, style: GoogleFonts.inter(fontSize: 10, fontWeight: FontWeight.w600, color: Colors.white.withAlpha(153), letterSpacing: 0.5)),
          ],
        ),
      ],
    );
  ...
  Widget _buildCategoryCard(ExpenseCategory cat, int catIndex) {
    final isExpanded = _expandedCategories.contains(catIndex) || _searchQuery.isNotEmpty;
    ...
    return Container(
      ...
      decoration: BoxDecoration(
        ...
        boxShadow: [BoxShadow(color: AppColors.ambientShadow, blurRadius: 24, offset: const Offset(0, 6))],
      ),
      child: Column(
        children: [
          // Header
            ...
            child: Padding(
              ...
              child: Row(
                children: [
                  Container(
                    width: 48, height: 48,
                    decoration: BoxDecoration(color: cat.color.withAlpha(26), borderRadius: BorderRadius.circular(14)),
                  ...
                  Expanded(
                    child: Column(
                      crossAxisAlignment: CrossAxisAlignment.start,
                      children: [
                        Text(cat.name, style: GoogleFonts.manrope(fontSize: 16, fontWeight: FontWeight.w700, color: AppColors.onSurface)),
                  ...
                  AnimatedRotation(
                    ...
                    child: Icon(Icons.keyboard_arrow_down_rounded, color: AppColors.onSurfaceVariant, size: 24),
                  ),
                ],
              ),
            ),
            ...
            secondChild: Padding(
              ...
              child: Column(
                children: subgroups.asMap().entries.map((entry) {
                  ...
                  return _buildSubgroupTile(entry.value, catIndex, realSubIndex >= 0 ? realSubIndex : entry.key);
                }).toList(),
              ),
            ),
  ...
  Widget _buildSubgroupTile(ExpenseSubgroup subgroup, int catIndex, int subIndex) {
    final key = '$catIndex-$subIndex';
    final isExpanded = _expandedSubgroups.contains(key) || _searchQuery.isNotEmpty;
            ...
            child: Padding(
              ...
              child: Row(
                children: [
                  Container(
                    width: 6, height: 6,
                    decoration: BoxDecoration(color: cat.color.withAlpha(153), shape: BoxShape.circle),
                  ),
                  const SizedBox(width: 12),
                  Expanded(
                    child: Text(
                  ...
                  Container(
                    ...
                    decoration: BoxDecoration(color: cat.color.withAlpha(20), borderRadius: BorderRadius.circular(100)),
                    child: Text('${subgroup.items.length}', style: GoogleFonts.inter(fontSize: 11, fontWeight: FontWeight.w700, color: cat.color)),
                  ),
                  ...
                  AnimatedRotation(
                    ...
                    child: Icon(Icons.keyboard_arrow_down_rounded, color: AppColors.onSurfaceVariant.withAlpha(128), size: 20),
                  ),
                ],
              ),
            ),
            ...
            secondChild: Padding(
              ...
              child: Column(
                children: subgroup.items.asMap().entries.map((entry) {
                  return _buildItemRow(entry.value, cat.color, catIndex, subIndex, entry.key);
                }).toList(),
              ),
            ),
            crossFadeState: isExpanded ? CrossFadeState.showSecond : CrossFadeState.showFirst,
  ...
  // ── Item Row (Level 3) ─────────────────────────────────────────────────────
  Widget _buildItemRow(ExpenseSubcategory item, Color color, int catIndex, int subIndex, int itemIndex) {
